...
// Pure React + CSS. No three.js, no motion. RAF loop drives a translate3d
// transform on a duplicated logo list for a seamless marquee.
...
export type NodeLogoItem = LogoItemBase & {
  node: ReactNode;
  src?: never;
};

export type ImageLogoItem = LogoItemBase & {
  src: string;
  alt?: string;
  srcSet?: string;
  sizes?: string;
  ...
  const renderLogoItem = useCallback(
    (item: LogoItem, key: Key) => {
      ...
      const content = isNodeItem ? (
        ...
        // External image marquee branch — next/image is overkill for a
        // 28–48px logo that may come from any origin. Keep a plain <img>.
...
LogoLoopInner.displayName = "LogoLoopInner";

export const LogoLoop = memo(LogoLoopInner);
